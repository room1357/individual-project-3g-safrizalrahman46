...
// Impor permission_handler sudah tidak diperlukan lagi
...
// (Kelas ExportPdf tidak diubah sama sekali)
class ExportPdf {
  ...
  // ... Widget bantu dan formatter lainnya
  ...
  static pw.Widget _expenseTable(List<Expense> list) {
    ...
    return pw.TableHelper.fromTextArray(
      headers: headers,
      data: data,
      headerDecoration: const pw.BoxDecoration(color: pdf.PdfColor.fromInt(0xFFEFF5FF)),
      headerStyle: pw.TextStyle(fontWeight: pw.FontWeight.bold, color: pdf.PdfColors.blue900),
  ...
  static String _formatDateTime(DateTime d) => '${_formatDate(d)} ${d.hour.toString().padLeft(2, '0')}:${d.minute.toString().padLeft(2, '0')}';
}
...
class ExportCsv {
  ...
  static Future<void> exportFromList(
    ...
    if (kIsWeb) {
      ...
      // --- DIKEMBALIKAN KE LOGIKA "SHARE" ---
      // 📱 ANDROID/iOS: simpan ke direktori sementara lalu buka dialog "Share"
      ...
      // Memanggil dialog "Share" untuk file tersebut
      await Share.shareXFiles(
        [XFile(filePath)], 
        text: 'Ini data pengeluaranku'
      );
      // --- BATAS PERUBAHAN ---
    }
  }

  // =================== Builder dan Helper CSV (tidak berubah) ===================
